...
- `OpportunitySelector.isActiveStage()` validates a Stage update against the org-wide *active* `Opportunity.StageName` picklist values. It does not account for multiple Sales Processes / Opportunity record types, where the set of valid Stages can legitimately differ per record type. This demo's requirement doesn't call for multiple sales processes, so the simpler org-wide check is acceptable here, but a production implementation touching a multi-process org would need to validate against the Stage values valid for the specific Opportunity's record type/sales process, not just the global active set.
